...
- `--no-filter` : выключить спам-фильтр в отчёте `queries` (см. ниже)
...
## Спам-фильтр в `queries`

Накрутка от конкурентов/ботов (показы по мусорным фразам, 0 кликов на топ-позициях) засирает реальную картину. Скрипт по умолчанию прячет такие запросы из таблицы и показывает их сводным 🚫-блоком ниже, с причиной. Эвристики: позиция ≤ 12 + показов ≥ 3 + 0 кликов; либо слитная кириллическая опечатка ≥ 14 букв; либо попадание под `SEO_BLOCKLIST` regex из `.env` (например `SEO_BLOCKLIST=skupik|казино`). Выключается `--no-filter`.
